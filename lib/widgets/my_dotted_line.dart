...
class MyDottedLine extends StatefulWidget {
  ...
  const MyDottedLine({
    ...
    this.child,
         ...
         'width or height or child must not be null',
       );
...
class _MyDottedLineState extends State<MyDottedLine> {
  ...
  CustomPaint dashPath({double? width, double? height}) {
    return CustomPaint(
      size: Size(
        _isEmpty(width) ? widget.strokeWidth : width!,
      ...
      Path path = Path()
        ..addRRect(
          RRect.fromLTRBAndCorners(
            ...
            size.height,
            topLeft: Radius.circular(
              corner != null ? corner!.leftTopCorner : 0.0,
            ),
            topRight: Radius.circular(
              corner != null ? corner!.rightTopCorner : 0.0,
            ),
            bottomLeft: Radius.circular(
              corner != null ? corner!.leftBottomCorner : 0.0,
            ),
            bottomRight: Radius.circular(
              corner != null ? corner!.rightBottomCorner : 0.0,
            ),
          ),
        );
